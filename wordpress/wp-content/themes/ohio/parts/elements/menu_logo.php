	...
	<a class="branding-title titles-typo -undash" href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
		...
		<div class="fixed-logo">
			<?php if ( $logo_for_fixed_as_image && ( $logo_for_fixed['default'] || $logo_for_fixed['retina'] ) ): ?>
				<img src="<?php echo esc_url( ( $logo_for_fixed['default'] ) ? $logo_for_fixed['default'] : $logo_for_fixed['retina'] ); ?>" class="<?php if ( $is_dark_scheme ) { echo ' main-logo'; }  ?><?php if ( $logo['have_vector'] ) { echo ' svg-logo'; } ?><?php if ( $logo_for_fixed['retina'] ) { echo ' srcset="' . $logo_for_fixed['retina'] . ' 2x"'; } ?> alt="<?php echo esc_attr( $logo_text ); ?>">
